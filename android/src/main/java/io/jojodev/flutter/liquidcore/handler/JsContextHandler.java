...
public class JsContextHandler implements MethodChannel.MethodCallHandler, EventChannel.StreamHandler {
    ...
    private JsContextHandler(Registrar registrar) {
        jsContextChannel = new MethodChannel(registrar.messenger(), LiquidcorePlugin.NAMESPACE + "/jscontext");
        jsContextChannel.setMethodCallHandler(this);

        EventChannel contextException = new EventChannel(registrar.messenger(), LiquidcorePlugin.NAMESPACE + "/jscontextException");
    ...
    private void handleMethodCall(MethodCall call, final MethodChannel.Result result) {
        ...
        if ("evaluateScript".equals(call.method)) {
            ...
            result.success(LiquidcorePlugin.convertToDartObject(jsContext.evaluateScript(script, sourceURL, startingLineNumber)));
            ...
            result.success(LiquidcorePlugin.convertToDartObject(jsContext.property(prop)));
            ...
            if ("function".equals(type)) {
                ...
                final JSFunction dartCb = new JSFunction(jsContext, "dartCb") {
                    ...
                    public void dartCb(final JSFunction resolve, final JSFunction error, Object... args) {
                        Map<String, Object> arguments = buildArguments(contextId, LiquidcorePlugin.convertToDartObjects(args));
